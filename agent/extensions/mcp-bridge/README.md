...
Canonical host tools are indexed into short-lived snapshots at reconciliation boundaries: before teardown, before synchronous candidate registration, and after that batch. Ownership, collision, and activation checks reuse the appropriate snapshot rather than rescanning Pi's registry per tool. The bridge caches its first canonically accepted connector `sourceInfo` identity for the runtime, so loss of all visible historical provenance cannot make a later foreign registration appear connector-owned. A post-registration snapshot remains authoritative: it determines whether Pi accepted, filtered, or gave another source precedence to each candidate. Dynamic allowlist reactivation is reconciled once after the batch and again on superseded/error cleanup when needed.
...
- Focused loopback-fixture tests cover normal-host and allowlist `A,B → B,C` reconciliation, bounded canonical-registry reads, source-aware first-registration-wins behavior, stable collision mappings and call targets, provenance loss, host-filtered names, serialized reconnect/teardown races, prompt abort of non-responsive discovery during disable/shutdown, superseded command notifications, successful empty catalogues, malformed-config disable, text truncation, temp-file markers, image preservation, and error formatting.
